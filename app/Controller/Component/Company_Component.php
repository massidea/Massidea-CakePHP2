...
class Company_Component extends Component {
	...
	public function setCompaniesForSave($companies) {
		...
		$existingCompanies = $this->extractNewCompanies($companyList);
...
// 		$this->_existingCompanies = $existingCompanies;
	...
	public function extractNewCompanies($companyList) {
		$companyInstance = ClassRegistry::init('RelatedCompany');
		foreach ($companyList as $company) {
			$tagExists = $companyInstance->find('count', array(
					'conditions' => array(
						'name' => $company
			)
			));
			if(!$tagExists) {
				$this->_newCompanies[] = $company;
			}
		}
		return $this;
	}
