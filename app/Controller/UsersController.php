...
class UsersController extends AppController {
	public $name = 'Users';
	public $components = array('RequestHandler', 'Email', 'Token', 'Recaptcha.Recaptcha');
	public $helpers = array('Time', 'Recaptcha.Recaptcha');
...
// 			debug($this->request->data); die;
			// Validate fields
			$validData = $this->User->saveAll($this->request->data['User'], array('validate' => 'only'));
// 			if($this->data['User']['agreement']) {
				if($validData) {
					// Save user data
					$userSaved = $this->User->saveAll($this->data, array('validate' => false));
	
					$signupError = null;
					if($userSaved) {
						// Reformat array structure for saving multiple key-value pairs
						$this->request->data['Profile'] = $this->__reformatProfileData($this->User->id, $this->data['Profile']);
						// Save profile data
